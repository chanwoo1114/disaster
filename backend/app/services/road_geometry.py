...
class RoadGeometry:
    ...
    @staticmethod
    def _convert_to_features(gdf: gpd.GeoDataFrame, properties_keys: List[str]) -> List[Dict]:
        features = []
            ...
            for key in properties_keys:
                if key in row:
                    ...
                    if isinstance(value, (int, float)):
                        feature["properties"][key] = int(value) if key == 'link_id' else bool(value)
                    else:
    ...
    def _create_micro_link(
        ...
        max_lanes = int(filtered_roads['LANES'].max()) if 'LANES' in filtered_roads.columns else 2
